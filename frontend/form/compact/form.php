...
<?php
if (isset($data->config['form_text_requiredfields']) && $data->config['form_text_requiredfields'] != null) { ?>
  <div class="pull-right">
    <label>
      <i><?= $data->config['form_text_requiredfields'] ?></i>
    </label>
  </div>
    <?php
} ?>
  ...
  require(['jquery'], function($) {
    require(['domReady!'], function() {
      ...
      $('#core_ui_form_container_<?=$data->config['form_id']?>').bind('submit', function() {
        ...
        let url = '<?=$data->config['form_action']?><?php if (str_contains($data->config['form_action'], '?')) { ?>&template=json<?php } else {  ?>?template=json<?php } ?>';

        let formData = new FormData($('#core_ui_form_container_<?=$data->config['form_id']?> form')[0]);
