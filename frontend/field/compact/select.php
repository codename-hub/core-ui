...
if ($data['field_required']) { ?>
  ...
  <div class="col-lg-9">
      ...
      <?php
if ($data['field_readonly'] && $data['field_required'] && !is_array($data['field_value'])) { ?>
...
if (isset($data['field_multiple']) && is_bool($data['field_multiple']) && $data['field_multiple']) {
    echo '[]';
} ?>"
      ...
      <?php
if (isset($data['field_onchange'])) { ?>onchange="<?= $data['field_onchange'] ?>"<?php
} ?>
      <?php
  ...
  if (isset($data['field_multiple']) && is_bool($data['field_multiple']) && $data['field_multiple']) {
    ...
    >
        <?php
    if (!isset($data['field_multiple']) || !$data['field_multiple']) { ?>
          <option
            value="" <?php
      if (is_string($data['field_value']) && strlen($data['field_value']) == 0) { ?>selected<?php
      } ?>><?= app::getTranslate()->translate('CRUD.PLEASE_SELECT') ?></option>
            <?php
    } ?>
