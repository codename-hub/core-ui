...
app::getResponse()->requireResource('css', 'assets/plugins/datetimepicker/jquery.datetimepicker.min.css');
app::getResponse()->requireResource('js', 'assets/plugins/datetimepicker/jquery.datetimepicker.full.js');
?>
...
<div class="form-group">
    ...
    <div class="col-lg-9" id="<?=$data['field_id']?>_datetimepicker_parent">
        ...
        />
        <?php if (!$data['field_readonly'] && (!isset($data['field_allowblank']) || ($data['field_allowblank'] == true))) {?>
        <button class="icon-remove-sign" type="button" onclick="$('#<?=$data['field_id']?>').val('');" style="position: absolute;
            top: 0;
            bottom: 0;
            right: 15px;
            width: 26px;
...
<script>
  ...
  $('#<?=$data['field_id']?>').datetimepicker({
    ...
    inline: <?= (isset($data['field_inline']) && $data['field_inline'] == true) ? 'true' : 'false' ?>,
    timepicker: <?= (isset($data['field_timepicker']) && $data['field_timepicker'] == false) ? 'false' : 'true' ?>,
    format: '<?= isset($data['field_format']) ? $data['field_format'] : 'YYYY-MM-DD HH:mm:ss' ?>',
    formatDate: '<?= isset($data['field_formatdate']) ? $data['field_formatdate'] : 'YYYY-MM-DD' ?>',
    formatTime: '<?= isset($data['field_formattime']) ? $data['field_formattime'] : 'HH:mm:ss' ?>',
    defaultDate: new Date(),
    validateOnBlur: true,
    scrollMonth: false,
    mask: '<?= isset($data['field_mask']) ? $data['field_mask'] : '9999-99-99 29:59:59' ?>',
    // defaultSelect: true,
