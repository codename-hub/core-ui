...
use codename\core\frontend\buttonGroup;
use codename\core\frontend\buttonLink;
use codename\core\frontend\buttonMenu;
use codename\core\frontend\link;
...
if ($enable_top_bar) { ?>
  <div class="row padded">
      <?php
      if ($enable_actions_top) { ?>
        <div class="btn-group">
            <?php
            if (is_array($actions_top)) { ?>
                <?php
                foreach ($actions_top as $key => $value) { ?>
                    <?php
                    if (isset($value['items']) && is_array($value['items'])) { ?>
                      <div class="btn-group pull-right">
                        <button class="btn btn-xs btn-success dropdown-toggle" data-toggle="dropdown"><?= app::getTranslate()->translate("BUTTON.BTN_" . $key) ?>
                        ...
                        <ul class="dropdown-menu">
                            ...
                            foreach ($value['items'] as $itemKey => $itemValue) {
                                ...
                                ?>
                              <li>
                                <a href="/?<?= http_build_query($requestParams) ?>" class="<?= $itemValue['btnClass'] ?? '' ?>">
                                  <i class="<?= $itemValue['icon'] ?>"></i> <?= isset($itemValue['translationkey']) ? app::getTranslate()->translate($itemValue['translationkey']) : app::getTranslate()->translate("BUTTON.BTN_" . $itemKey) ?></a>
                              </li>
                                <?php
                            } ?>
                        </ul>
                        ...
                        ?>
                      <a href="/?<?= http_build_query($requestParams) ?>" class="btn btn-xs <?= $value['btnClass'] ?>">
                        <i class="<?= $value['icon'] ?>"></i> <?= isset($value['translationkey']) ? app::getTranslate()->translate($value['translationkey']) : app::getTranslate()->translate("BUTTON.BTN_" . $key) ?></a>
                        <?php
                    } ?>
                    <?php
                } ?>
            ...
            if ($enable_filters) { ?>
              <div class="btn-group">
                <button class="btn btn-success btn-xs dropdown-toggle" data-toggle="dropdown">
                  <i class="icon icon-filter"></i> <?= app::translate('BUTTON.BTN_FILTER') ?>
                  <span class="caret"></span>
                </button>
                <ul class="dropdown-menu">
                    <?php
                    foreach ($filters_unused as $field => $filter) { ?>
                        <?php
                        if ($field != '_search') { ?>
                          <li
                            class="btnAdder" data-fieldname="<?= app::getTranslate()->translate('DATAFIELD.' . $field) ?>" data-filterfield="<?= $field ?>" data-multiple="<?= (isset($filter['multiple']) && $filter['multiple']) ? 'true' : 'false' ?>" <?php
                          if (isset($filter['filtertype'])) { ?>data-filtertype="<?= $filter['filtertype'] ?>"<?php
                          } ?> <?php
                          if (isset($filter['filteroptions'])) { ?>data-filteroptions="<?= htmlspecialchars(json_encode($filter['filteroptions'])) ?>" <?php
                          } ?>>
                            <a data-filterfield="<?= $field ?>">
                              <i class="icon icon-plus"></i> <?= app::getTranslate()->translate('DATAFIELD.' . $field) ?></a>
                          </li>
                            <?php
                        } ?>
                        <?php
      ...
      <?php
      if ($enable_actions_bulk) { ?>
        <div class="btn-group pull-right">
          <button class="btn btn-xs btn-success dropdown-toggle" data-toggle="dropdown">Bulk actions
            <span class="caret"></span>
          </button>
          <ul class="dropdown-menu">
              <?php
              foreach ($actions_bulk as $key => $value) { ?>
                <li>
                  <a class="btnBulk" href="/?context=<?= $value['context'] ?>&view=<?= $value['view'] ?><?= (array_key_exists('action', $value) ? '&action=' . $value['action'] : '') ?>">
                    <i class="<?= $value['icon'] ?>"></i> <?= app::getTranslate()->translate("BUTTON.BTN_" . $key) ?></a>
                </li>
                  <?php
              } ?>
          </ul>
...
<form method="GET">
  ...
  <input type="hidden" name="crud_pagination_page_prev" value="<?= $crud_pagination_page ?>" />
    ...
    <?php
    if ($enable_filters || $enable_displayfieldselection) { ?>

      <div class="row padded">

        <!-- select displayed columns/fields -->
          <?php
          if ($enable_displayfieldselection) { ?>
            <div id="crudFieldSelection">
                <?php
                if (is_array($display_availablefields) && is_array($display_selectedfields)) {
                    // to preserve order:
                  ...
                  <div class="col-md-2 alert-info">
                    <select class="chzn-select" name="display_selectedfields[]" multiple="multiple">
                        <?php
                        foreach ($orderedDisplayFields as $field) { ?>
          ...
          if ($enable_filters) { ?>
            <div id="crudFilterList">
                ...
                if ($enable_seach_bar) { ?>
                  <div class="col-md-2 alert-info">
                      <?= app::translate('BUTTON.BTN_SEARCH') ?>:
                    <br />
                ...
                <?php
                if (is_array($filters_used)) { ?>
                    <?php
                    foreach ($filters_used as $field => $filter) { ?>
                        <?php
                        if ($field == 'search') {
                            continue;
                        } ?>
                      <div class="col-md-2 alert-info">
                          <?= app::getTranslate()->translate('DATAFIELD.' . $field) ?>:
                        <br />
                          <?php
                          if (isset($filters_unused[$field]['filteroptions'])) { ?>
                            <select class="chzn-select" style="width:100px;" name="<?= $crud_filter_identifier ?>[<?= $field ?>]<?= $filters_unused[$field]['multiple'] ? '[]' : '' ?>" <?= $filters_unused[$field]['multiple'] ? 'multiple="multiple"' : '' ?>>
                                <?php
                                foreach ($filters_unused[$field]['filteroptions'] as $key => $value) { ?>
                                  <option
                                    value="<?= $key ?>"
                                    <?php
                                    if (is_array(app::getRequest()->getData($crud_filter_identifier)[$field])) {
                                        if (in_array($key, app::getRequest()->getData($crud_filter_identifier)[$field])) {
                                            echo('selected="selected"');
                                        }
                                    } elseif (app::getRequest()->getData($crud_filter_identifier)[$field] == $key) {
                                        echo('selected="selected"');
                                    } ?>>
                                      <?= $value ?>
                                  </option>
                              ...
                              <?php
                          } elseif (isset($filters_unused[$field]['filtertype']) && $filters_unused[$field]['filtertype'] == 'date_range') { ?>
      ...
      <input type="submit" class="btn btn-xs btn-success" value="<?= app::translate('BUTTON.BTN_FILTER') ?>" />
        ...
        if ($enable_export === true) { ?>
          <button type="submit" class="btn btn-xs btn-warning pull-right" name="action" value="export">
            <i class="icon-download-alt"></i><?= app::translate('BUTTON.BTN_EXPORT') ?></button>
            <?php
        } ?>
          ...
          foreach ($data_rows as $row) { ?>
            <tr <?= $row['__modifier'] ?? '' ?>>
                ...
                if ($enable_actions_bulk) { ?>
                  <td>
                      <?php
                      if ($row[$model->getPrimaryKey()] != null) { ?>
                      ...
                      if ($row[$model->getPrimaryKey()] != null) { ?>
                        <div class="btn-group">
                          <button type="button" class="btn btn-xs btn-default dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                            <i class="icon icon-cogs"></i> <?= app::getTranslate()->translate("BUTTON.BTN_OPTIONS") ?>
                            <span class="caret"></span>
                          </button>
                          <ul class="dropdown-menu">
                              <?php
                              foreach ($actions_element as $key => $value) {
                ...
                foreach ($data_fields as $field) { ?>
                  <td>
                      <?php
                      if ($enable_actions_field && array_key_exists($field, $actions_field)) {
      ...
      <tfoot>
        <tr>
          <td colspan="<?= count($data_fields) + 2 ?>" style="text-align:center">
            ...
            <br />
              <?php
              if ($crud_pagination_pages > 1) { ?>
                  <?php
                  for ($page = 1; $page <= $crud_pagination_pages; $page++) { ?>
                    <button
                      type="submit" name="crud_pagination_page" value="<?= $page ?>" class="btn btn-xs <?php
                    if ($page == $crud_pagination_page) { ?> btn-success <?php
                    } else { ?> btn-default <?php
                    } ?>"
                    ><?= $page ?></button>
                      <?php
                  } ?>
              ...
              $paginationOptions = [0, 10, 25, 50, 100, 200, 500];
  ...
  require(['jquery'], function($) {
    ...
    require(['domReady!'], function() {
      ...
      $(document).delegate('.btnAdder', 'click', function() {
        let filterfield = $(this).data('filterfield');
        let fieldname = $(this).data('fieldname');
        let filteroptions = $(this).data('filteroptions');
