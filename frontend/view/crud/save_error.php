...
<script>
  require(['domReady!', 'jquery'], function() {
    ...
    $("#<?=$error['__IDENTIFIER']?>error").html('<?php echo($error['__DETAILS'][0]['__TEXT'] ?? (isset($error['__DETAILS'][0]['__CODE']) ? app::translate($error['__DETAILS'][0]['__CODE']) : app::translate($error['__CODE'])))?>');
      <?php } ?>
  });
</script>
